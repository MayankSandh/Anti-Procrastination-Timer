from tkinter import Tk, LabelFrame, Label, Entry, Frame, PhotoImage, Button, FLAT, X, LEFT, RIGHT, NORMAL, DISABLED, END, Menu
...
def themeChanger(rootcolor, timerframecolor, timerframefg, buttonbg, buttonfg, buttonhbg, buttonhfg):
    ...
    SleepLabel.config(bg = timerframecolor, fg = timerframefg)
...
def displayNotification(shutdownMode):
    if shutdownMode == "signoff":
        cmd("shutdown /l")
...
def cancelTimer(): # FUNCTION OF THE CANCEL BUTTON 
    EnableAllEntries()
    changeHourEntry("")
    changeMinuteEntry("")
    changeSecondEntry("")
...
SleepLabel = Label(TimerFrame, text = "SLEEP_ENABLED!", fg = "black", bg = "white", font = "Helvetica 17 bold")
SleepLabel.grid(row = 2, column = 0)
...
ClearButton = Button(ButtonFrame, image = Cancel_image, bg = "#b7e9f7", relief = FLAT, command = cancelTimer, activebackground = "#b7e9f7", bd = 0)
